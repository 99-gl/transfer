# Modified copy of Search-R1/verl/trainer/ppo/ray_trainer.py
...
def _batch_item_value(non_tensor_batch, key, index, default=None):
    value = non_tensor_batch.get(key, default)
    if value is default:
        return default
    try:
        return _json_safe(value[index])
    except Exception:
        return _json_safe(value)


def _extract_question(non_tensor_batch, index):
    extra_info = _batch_item_value(non_tensor_batch, 'extra_info', index, {})
    if isinstance(extra_info, dict):
        for key in ('question', 'query', 'prompt'):
            if key in extra_info:
                return extra_info[key]

    prompt = _batch_item_value(non_tensor_batch, 'prompt', index)
    if isinstance(prompt, list) and prompt:
        last_turn = prompt[-1]
        if isinstance(last_turn, dict):
            return last_turn.get('content', last_turn)
        return last_turn
    return prompt


def _extract_ground_truth(non_tensor_batch, index):
    reward_model = _batch_item_value(non_tensor_batch, 'reward_model', index, {})
    if isinstance(reward_model, dict):
        return reward_model.get('ground_truth', reward_model)
    return reward_model


def _extract_answer(response):
    matches = list(re.finditer(r'<answer>(.*?)</answer>', response or '', re.DOTALL))
    if not matches:
        return None
...
def _decode_validation_response(tokenizer, batch, index):
    import torch

    prompt_ids = batch.batch['prompts'][index]
    prompt_length = prompt_ids.shape[-1]
    valid_prompt_length = batch.batch['attention_mask'][index][:prompt_length].sum()

    response_ids = batch.batch['responses'][index]
    valid_response_length = batch.batch['attention_mask'][index][prompt_length:].sum()
    valid_response_ids = response_ids[:valid_response_length]

    response = tokenizer.decode(valid_response_ids)
    sequence = tokenizer.decode(torch.cat((prompt_ids[-valid_prompt_length:], valid_response_ids)))
    return response, sequence


def _validation_record(tokenizer, batch, reward_tensor, index, global_index):
    response, sequence = _decode_validation_response(tokenizer, batch, index)
    non_tensor_batch = batch.non_tensor_batch
    score = reward_tensor[index].sum().detach().cpu().item()
    return {
        'type': 'sample',
        'index': int(global_index),
        'dataset_index': _batch_item_value(non_tensor_batch, 'index', index),
        'data_source': _batch_item_value(non_tensor_batch, 'data_source', index, 'unknown'),
        'question': _extract_question(non_tensor_batch, index),
        'response': response,
        'answer': _extract_answer(response),
        'ground_truth': _extract_ground_truth(non_tensor_batch, index),
        'score': float(score),
        'trajectory': sequence,
    }
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        val_dataset_size = len(self.val_dataset) if hasattr(self, 'val_dataset') else None
        val_batch_size = self.config.data.val_batch_size
        val_batches = len(self.val_dataloader)
        val_num_to_run = val_batches * val_batch_size
        trace_dir = self.config.trainer.get('validation_trace_dir', 'outputs/eval_traces')
        os.makedirs(trace_dir, exist_ok=True)
        timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        project_name = _safe_filename_part(self.config.trainer.project_name)
        experiment_name = _safe_filename_part(self.config.trainer.experiment_name)
        trace_path = os.path.join(
            trace_dir,
            f"{project_name}_{experiment_name}_step{self.global_steps}_{timestamp}.jsonl",
        )
        ...
        trace_meta = {
            'type': 'meta',
            'source_file': 'Search-R1/verl/trainer/ppo/ray_trainer.py',
            'created_at': datetime.datetime.now().isoformat(timespec='seconds'),
            'global_steps': int(self.global_steps),
            'trace_path': trace_path,
            'val_dataset_size_after_filter': val_dataset_size,
            'val_batch_size': val_batch_size,
            'val_dataloader_batches': val_batches,
            'val_num_to_run': val_num_to_run,
            'note': 'val_num_to_run equals len(val_dataloader) * val_batch_size; drop_last=True may omit the tail of the validation dataset.',
            'config': _json_safe(OmegaConf.to_container(self.config, resolve=True)),
            'generation_config': _json_safe(gen_config.__dict__),
        }
        with open(trace_path, 'w', encoding='utf-8') as trace_file:
            trace_file.write(json.dumps(trace_meta, ensure_ascii=False) + '\n')
            ...
            for test_data in self.val_dataloader:
                ...
                with open(trace_path, 'a', encoding='utf-8') as trace_file:
                    for i in range(len(test_batch)):
                        record = _validation_record(self.tokenizer, test_batch, reward_tensor, i, trace_record_count)
            ...
            for batch_dict in self.val_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with open(trace_path, 'a', encoding='utf-8') as trace_file:
                        for i in range(len(test_batch)):
                            record = _validation_record(self.tokenizer, test_batch, reward_tensor, i, trace_record_count)
                            record['timing'] = _json_safe(timing_raw)
                            trace_file.write(json.dumps(_json_safe(record), ensure_ascii=False) + '\n')
                            trace_record_count += 1
