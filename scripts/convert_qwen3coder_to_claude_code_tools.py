...
def map_tool_call(call: dict[str, Any]) -> tuple[str, dict[str, Any]] | None:
    """Return (source_tool_name, mapped_call), or None for submit."""
    ...
    if source_name == "bash":
        ...
        return source_name, {"type": "function", "function": {"name": "Bash", "arguments": {"command": command}}}
    ...
    if command == "view":
        ...
        if view_range is not None:
            ...
            mapped_args["offset"] = view_range[0]
            mapped_args["limit"] = view_range[1] - view_range[0] + 1
        return source_name, {"type": "function", "function": {"name": "Read", "arguments": mapped_args}}

    if command == "create":
        content = arguments.get("file_text")
        if not isinstance(content, str):
            raise ConversionError("str_replace_editor(create) has no string file_text")
        return source_name, {
            "type": "function",
    ...
    if command == "str_replace":
        ...
        if not isinstance(old_string, str) or not isinstance(new_string, str):
            raise ConversionError("str_replace_editor(str_replace) requires string old_str and new_str")
        return source_name, {
            "type": "function",
            "function": {
                "name": "Edit",
...
def convert_messages(messages: Any, stats: Counter[str]) -> list[dict[str, Any]]:
    ...
    for index, message in enumerate(source_messages):
        ...
        if role == "assistant":
            ...
            for call in message.get("tool_calls") or []:
                source_name, mapped = map_tool_call(as_dict(call, "tool call")) or ("submit", None)
                if mapped is None:
                    pending.append((source_name, None))
                    stats["submit_calls_removed"] += 1
                    submit_call_count += 1
                else:
                    pending.append((source_name, mapped["function"]["name"]))
        ...
        if role == "tool":
            ...
            expected_source, target_name = pending.popleft()
            ...
            elif has_later_assistant(source_messages, index):
    ...
    if pending:
        ...
        if any(target_name is not None for _, target_name in unreturned):
            raise ConversionError(f"Missing tool result for calls: {unreturned!r}")
        stats["terminal_submit_without_result"] += len(unreturned)
