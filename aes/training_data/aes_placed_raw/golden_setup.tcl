...
#   innovus -nowin -files golden_setup.tcl
#
# Generates only setup.golden.json for the aes_placed_raw checkpoint.
# Run report_setup.tcl first so the report Beginpoint is available.

set SCRIPT_DIR [file dirname [file normalize [info script]]]
set CHECKPOINT /home/host/aes_baseline/outputs/aes_placed_raw.enc
set DATABASE /home/host/aes_baseline/outputs/aes_placed_raw.enc.dat
set REPORT_FILE [file join $SCRIPT_DIR setup.rpt]
set GOLDEN_FILE [file join $SCRIPT_DIR setup.golden.json]

if {![file exists $CHECKPOINT] || ![file isdirectory $DATABASE]} {
    error "Checkpoint pair not found: $CHECKPOINT and $DATABASE"
}
if {![file exists $REPORT_FILE]} {
    error "Missing $REPORT_FILE. Run report_setup.tcl first."
}
...
if {$paths eq "0x0" || $paths eq ""} {
    error "No setup timing path returned for $CHECKPOINT"
}
...
puts $fh "{\"checkpoint\":\"aes_placed_raw.enc\",\"stage\":\"aes_placed_raw\",\"analysis\":\"setup\",\"status\":\"ok\",\"startpoint\":\"$startpoint\",\"sta_startpoint_pin\":\"$sta_startpoint\",\"endpoint\":\"$endpoint\",\"slack_ns\":$slack,\"path_group\":\"$path_group\"}"
close $fh
